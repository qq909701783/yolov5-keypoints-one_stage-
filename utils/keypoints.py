...
BODY_PARTS_PAF_IDS = [[0,1],[1,2],[2,3],[3,0]]

def extract_keypoints(heatmap, all_keypoints, total_keypoint_num):
    heatmap[heatmap < 0.6] = 0 # 256 * 256
    heatmap_with_borders = np.pad(heatmap, [(2, 2), (2, 2)], mode='constant') # 260 * 260
    ...
    for i in range(len(keypoints)):
        ...
        for j in range(i+1, len(keypoints)):
            if math.sqrt((keypoints[i][0] - keypoints[j][0]) ** 2 +
                ...
                suppressed[j] = 1
        keypoint_with_score_and_id = (keypoints[i][0], keypoints[i][1], heatmap[keypoints[i][1], keypoints[i][0]], total_keypoint_num + keypoint_num)
...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=20, min_paf_score=0.005):
    pose_entries = []
    ...
    for part_id in range(len(BODY_PARTS_PAF_IDS)):
        ...
        # 根据获得的亲和评分，得到一个四肢列表。
        valid_limbs = np.where(np.logical_and(affinity_scores > 0, success_ratio > 0.05))[0]
